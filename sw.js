const CACHE='biicode-v5';
const APP=['./','./index.html','./manifest.json','./fix.js'];

const START_PRIVATE_OLD=`async function startPrivateApp(){if(booting)return;booting=true;try{currentUser=currentUser||((await db.auth.getUser()).data.user);await loadData();home()}finally{booting=false}}`;
...
const START_APP_OLD=`async function startApp(){const p=new URLSearchParams(location.search),bike=p.get('bike');if(bike){await publicMode(bike);return}const {data:s}=await db.auth.getSession();if(s.session?.user){currentUser=s.session.user;if(location.hash.includes('type=recovery')){showResetPassword();return}await startPrivateApp()}else showLogin()}`;
const START_APP_NEW=`async function startApp(){const p=new URLSearchParams(location.search),bike=p.get('bike');if(bike){await publicMode(bike);return}try{const {data:s}=await db.auth.getSession();if(s.session?.user){currentUser=s.session.user;if(location.hash.includes('type=recovery')){showResetPassword();return}await startPrivateApp()}else showLogin()}catch(e){console.error('BIICODE auth bootstrap:',e);showLogin()}}`;

function patchHtml(text){
  let patched=text;
  patched=patched.replace('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2','https://unpkg.com/@supabase/supabase-js@2');
  patched=patched.replace(START_PRIVATE_OLD,START_PRIVATE_NEW);
  patched=patched.replace(START_APP_OLD,START_APP_NEW);
