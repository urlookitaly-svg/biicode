...
(function(){
  ...
  function localBike(id){
    try{
      for(let i=0;i<localStorage.length;i++){
        const k=localStorage.key(i)||'';
        if(!k.startsWith('biicode_v3_'))continue;
        const rows=JSON.parse(localStorage.getItem(k)||'[]');
        if(Array.isArray(rows)){
          const b=rows.find(x=>x&&x.biicode_id===id);
          if(b)return b;
        }
      }
    }catch(e){}
  ...
  window.biicodeShare=async function(encoded){
    ...
    if(navigator.share){try{await navigator.share({title:'BIICODE',text:'Verifica questa bicicletta su BIICODE',url});return;}catch(e){}}
    try{await navigator.clipboard.writeText(url);alert('Link di verifica copiato.');}catch(e){alert(url);}
  };
  ...
  window.detail=async function(enc){
    ...
    window.openModal(`
      ...
      <h1>${esc(b.brand)} ${esc(b.model)}</h1>
      <div class="card">
        <div class="muted">BIICODE</div>
        <div style="font-size:24px;font-weight:900;margin-top:5px">${esc(b.biicode_id)}</div>
        ...
        <div style="font-size:20px;font-weight:800">${esc(b.frame_number||'-')}</div>
        <br>
        <div class="muted">ANNO · COLORE</div>
        <div style="font-size:18px;font-weight:800">${esc(b.year||'-')} · ${esc(b.color||'-')}</div>
  ...
  if(publicId){
    ...
    setTimeout(showPublic,0);setTimeout(showPublic,300);
  }
})();
